...
/**
 * fetchSheetData - Funcion generica para leer datos de cualquier hoja
 *
 * @param spreadsheetId - El ID unico del Google Sheet
...
async function fetchSheetData(
  ...
  // Si no hay API Key configurada, retornamos arreglo vacio
  // para que la pagina no truene, solo se muestra sin datos
  if (!apiKey) {
    console.warn(
      "[Google Sheets] No se encontro GOOGLE_SHEETS_API_KEY. Usando datos de respaldo."
    )
    return []
  }
  ...
  // Si no hay datos, retornamos arreglo vacio
  return data.values || []
 ...
 * A: nombre | B: descripcion | C: categoria | D: contacto | E: direccion
 *
 * La primera fila (encabezados) se salta porque el rango empieza en A2.
 * Si no hay API Key o falla la conexion, usa datos de respaldo del JSON local.
 */
export async function getEmpresas(): Promise<Empresa[]> {
  ...
  // Si no hay filas (sheet vacio o error), usamos datos de respaldo
  if (rows.length === 0) {
    const fallback = await import("@/data/empresas.json")
    return fallback.default as Empresa[]
  }
